...
import { getAllCategories } from '../data/mock-data';
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <header className="bg-white shadow-sm sticky top-0 z-30">
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <Link to="/" className="text-2xl font-bold text-primary flex items-center">
              <span className="bg-primary text-white rounded-lg py-1 px-2 mr-2">A</span>
              Atlassian Blog
            </Link>
            ...
            <nav className="hidden md:flex items-center space-x-6">
              <Link 
                to="/" 
                className={`hover:text-primary transition-colors ${location.pathname === '/' ? 'text-primary font-medium' : 'text-neutral-700'}`}
              >
                Trang chủ
              </Link>
              <Link 
                to="/latest" 
                className={`hover:text-primary transition-colors ${location.pathname === '/latest' ? 'text-primary font-medium' : 'text-neutral-700'}`}
              >
                Bài viết mới
              </Link>
              <div className="relative group">
                <button className="text-neutral-700 hover:text-primary transition-colors flex items-center">
                  Danh mục
                  <svg className="w-4 h-4 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7"></path>
                  </svg>
                </button>
                <div className="absolute left-0 mt-2 w-48 bg-white rounded-md shadow-lg overflow-hidden z-40 invisible group-hover:visible opacity-0 group-hover:opacity-100 transition-all duration-300">
        ...
        {/* Mobile Menu */}
        {mobileMenuOpen && (
          <div className="md:hidden bg-white border-t border-neutral-100 py-4 animate-slide-down">
            <div className="container mx-auto px-4 flex flex-col space-y-4">
              <Link 
                to="/" 
                className={`py-2 ${location.pathname === '/' ? 'text-primary font-medium' : 'text-neutral-700'}`}
              ...
              <Link 
                to="/latest" 
                className={`py-2 ${location.pathname === '/latest' ? 'text-primary font-medium' : 'text-neutral-700'}`}
                onClick={() => setMobileMenuOpen(false)}
              >
                Bài viết mới
              </Link>
        ...
        {/* Search Overlay */}
        {searchOpen && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-start justify-center z-40 pt-20 px-4 animate-fade-in">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl overflow-hidden">
              <div className="p-4 flex items-center border-b border-neutral-100">
                ...
                <button onClick={toggleSearch}>
                  <X size={20} className="text-neutral-500 hover:text-neutral-800" />
                </button>
              </div>
              <div className="p-4">
                <p className="text-neutral-500 text-sm">
                  {searchQuery.length > 0 
                    ? 'Chức năng tìm kiếm đang được phát triển. Vui lòng thử lại sau.' 
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            <div className="col-span-1 md:col-span-2">
              ...
              <p className="text-neutral-300 mb-4">
                Khám phá những xu hướng mới nhất, bài viết hấp dẫn và thông tin hữu ích từ các chuyên gia 
